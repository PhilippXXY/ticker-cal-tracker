...
### Information about the cloud deployment

- Usage of Google Cloud Run (Serverless)
- Usage of Google Cloud SGL for Postgre SQL
- Server location eu-west-2 (london)


### Automation
The CI/CD pipeline only allows PRs when type checks by `mypy`, linting checks by `ruff` and the tests have been completed successfully.
Once a PR to the main branch gets merged the Docker image gets build automatically, as well as a new release created, the image gets published to Cloud Run and the mkdocs github pages site gets updated.

### What else to mention

The website uses HTTPS.

As we use serverless computing we utitilse its functionality to autoscale.

The Cloud database backups automatically.
